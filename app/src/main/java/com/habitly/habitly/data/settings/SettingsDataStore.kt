...
class SettingsDataStore(private val context: Context) {
    ...
    companion object {
        ...
        val IGNORE_WELCOME_ENGINE_KEY = booleanPreferencesKey("ignore_welcome_engine")
    }
    ...
    suspend fun clearWelcomeCardState() {
        context.dataStore.edit { settings ->
            settings.remove(WELCOME_CARD_DATE_KEY)
            settings.remove(WELCOME_CARD_CATEGORY_ID_KEY)
            settings.remove(WELCOME_CARD_HABIT_ID_KEY)
            settings.remove(WELCOME_CARD_TEMPLATE_INDEX_KEY)
        }
    }

    val ignoreWelcomeEngine: Flow<Boolean> = context.dataStore.data
        .map { preferences ->
            preferences[IGNORE_WELCOME_ENGINE_KEY] ?: DefaultSettings.IGNORE_WELCOME_ENGINE
        }

    suspend fun setIgnoreWelcomeEngine(ignore: Boolean) {
        context.dataStore.edit { settings ->
            settings[IGNORE_WELCOME_ENGINE_KEY] = ignore
        }
    }
    ...
    suspend fun getExportedSettings(): ExportedSettings {
        ...
        return ExportedSettings(
            ...
            ignoreWelcomeEngine = preferences[IGNORE_WELCOME_ENGINE_KEY] ?: DefaultSettings.IGNORE_WELCOME_ENGINE
        )
    }
    ...
    suspend fun importSettings(settings: ExportedSettings) {
        context.dataStore.edit { preferences ->
            ...
            preferences[IGNORE_WELCOME_ENGINE_KEY] = settings.ignoreWelcomeEngine
        }
    }
}
...
data class ExportedSettings(
    ...
    val ignoreWelcomeEngine: Boolean = DefaultSettings.IGNORE_WELCOME_ENGINE
)
